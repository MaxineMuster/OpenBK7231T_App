...
const char pageScriptPart1[] = "<script type='text/javascript'>var firstTime,lastTime,onlineFor,req=null,onlineForEl=null,getElement=e=>document.getElementById(e);const st=getElement('state');function restoreState(){st.querySelectorAll('details').forEach((e) => {(e.id)&&(x=h.querySelector('#'+e.id))&&(x.open = e.open);})};function showState(){clearTimeout(firstTime),clearTimeout(lastTime),null!=req&&req.abort(),(st)&&(h=getElement(\"t_state\"))&&((req=new XMLHttpRequest).onreadystatechange=()=>{4==req.readyState&&\"OK\"==req.statusText&&((\"INPUT\"!=document.activeElement.tagName||\"number\"!=document.activeElement.type&&\"color\"!=document.activeElement.type)&&(saveState(),h.innerHTML=req.responseText,restoreState(),st.innerHTML=h.innerHTML),clearTimeout(firstTime),clearTimeout(lastTime),lastTime=setTimeout(showState,";
