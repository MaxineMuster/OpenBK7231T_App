...
commandResult_t NTP_Info(const void *context, const char *cmd, const char *args, int cmdFlags) {
    addLogAdv(LOG_INFO, LOG_FEATURE_NTP, "Server=%s", CFG_GetNTPServer());
    return CMD_RES_OK;
}
...
void NTP_Init() {
	...
	//cmddetail:"descr":"deprecated!! Please use CLOCK commands - clock_setTZ / CLOCK_SetTimeZoneOfs",
	//cmddetail:"fn":"CLOCK_SetTimeZoneOfs","file":"driver/drv_ntp.c","requires":"",
	//cmddetail:"examples":""}
    CMD_RegisterCommand("ntp_timeZoneOfs",CLOCK_SetTimeZoneOfs, NULL);
...
#if ENABLE_CLOCK_DST
	return g_ntpTime - g_timeOffsetSeconds - getDST_offset()*60;
#else
...
#endif
*/
    time=secsSince1900 - NTP_OFFSET;
    CLOCK_setDeviceTime(time);
    addLogAdv(LOG_INFO, LOG_FEATURE_NTP,"Unix time  : %u",(unsigned int)time);
/*
...
*/
void NTP_Server_Status(char *str,size_t len){
    if (g_synced == true)
	snprintf(str, len," (NTP: %s in sync)",CFG_GetNTPServer());
    else 
	snprintf(str, len, " (NTP: Syncing with %s ...)",CFG_GetNTPServer());
