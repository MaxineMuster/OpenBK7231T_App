...
#define NEO6M_UART_ADDR 0 // 0 - 3
#define NEO6M_UART_CMD_READ(addr) (0x58 | addr)
#define NEO6M_UART_CMD_WRITE(addr) (0xA8 | addr)
#define NEO6M_UART_REG_PACKET 0xAA
#define NEO6M_UART_PACKET_HEAD 0x55
#define NEO6M_UART_PACKET_LEN 23
...
//	int sret= sscanf (data,"$GPRMC,%2i%2i%2i.%i,A,%i.%i,%C,%i.%i,%C,%*s,%*s,%2i%2i%2i,%*s",&H,&M,&S,&SS,&BB,&BBB,b,&LL,&LLL,l,&DD,&MM,&YY);
	...
	char *p = strchr(data, ',');	// so we are after $GPRMC
...
//	int sret= sscanf (p+1,"%20i%20i%20i.%i,%c",&H,&M,&S,&SS,&NS);
	gpslocked =  (NS == 'A' );
	if (!gpslocked) {        
...
static void UART_WriteDisableNMEA(void) {
    ...
    for (int i = 0; i < 7; i++) {
	    for (int j = 0; i < sizeof(send[i]); j++) {
        	UART_SendByte(send[i][j]);
            }
    }
}
