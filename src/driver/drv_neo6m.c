...
static int UART_TryToGetNextPacket(void) {
	...
	char data[NEO6M_UART_RECEIVE_BUFFER_SIZE+5]={0};
        char DATA1[4],DATA2[9],DATA3[8]; // to deal with leading "0", use characters	DATA1: LAT/LONG; DATA2: minutes; DATA3: fraction of minutes
	...
	//	int sret= sscanf (data,"$GPRMC,%2i%2i%2i.%i,A,%i.%i,%C,%i.%i,%C,%*s,%*s,%2i%2i%2i,%*s",&H,&M,&S,&SS,&BB,&BBB,b,&LL,&LLL,l,&DD,&MM,&YY);
		...
		int NMEACS=0;
		p++ ; // so we are after the "$" of $GPRMC and can start calculation
		...
		p = strstr(data, "$GPRMC,")+6;	// so we are after $GPRMC
