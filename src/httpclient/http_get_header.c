...
//	if (conv < 35  || strcmp(gmt, "GMT") ){
	if (conv < 35  || strstr(date, "GMT") - date != 32 ){
ADDLOG_INFO(LOG_FEATURE_CMD, "  get_HTTP_Header -- Date:-line found but format is unknown");
...
int sendbuff = tcp_sndbuf(pcb);
 if (sendbuff){
	...
	err = tcp_output(pcb);
 	 ...
 	 ADDLOG_INFO(LOG_FEATURE_CMD, " get_HTTP_Header -- error getting buffer space (sendbuff=%i)",sendbuff);
...
// we only need the header to get date and time
    char request_template[] = "HEAD / HTTP/1.1\r\nHost: %s\r\nConnection: Close\r\n\r\n";
    int request_len;
    int socket_fd;
    int conv;
